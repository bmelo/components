   ...
   */
	public function renderManyManyDropDownListSelection()
	{
		for($i=0; $i < $this->max; $i++){ 
			$objPk = $this->_relatedModel->primaryKey();
			$isAssigned = $this->isAssigned($this->_relatedModel->$objPk);

			echo CHtml::openTag('div', array(
						'id' => 'div' . $i,
						'style' => $isAssigned ? '' : 'display:none;',
						));
			echo CHtml::dropDownList($this->getListBoxName()."[$i]",
					$isAssigned ? $this->_relatedModel->$objPk : 0,
					CHtml::listData(
						array_merge(
		...
		Yii::app()->getClientScript()->registerScript('addbutton', $jsadd); 
		Yii::app()->getClientScript()->registerScript('subbutton', $jssub); 
		if( $this->allowEmpty !== false ){
			Yii::app()->getClientScript()->registerScript('addFirst', '$("#add").click()'); 
		}
	...
	public function run()
	{
		...
		if($this->showAddButton !== false) 
		{
			if(!isset($this->returnLink) or $this->returnLink == "")
				$this->returnLink = $this->_model->tableName() . "/create";

			echo CHtml::Link(
					is_string($this->showAddButton) ?
					$this->showAddButton : 'New' , array(
						$this->_relatedModel->tableName() . "/" . $this->createAction, 
